...
def get_available_stocks():
    """Récupère la liste des tables (actions) disponibles dans Supabase"""
    try:
        # Récupérer les noms des tables depuis information_schema
        response = supabase.rpc('get_table_names').execute()
        
        # Si la RPC n'existe pas, on utilise une liste statique
        # Liste basée sur vos fichiers CSV
        stocks = [
            ...
            'BAX', 'BBWI', 'BBY', 'BDX', 'BEN', 'BF_B', 'BG', 'BIIB', 'BIO', 'BK',
            'BKNG', 'BKR', 'BLK', 'BMY', 'BR', 'BRK_B', 'BRO', 'BSX', 'BWA', 'BX',
            ...
            'CRL', 'CRM', 'CSCO', 'CSGP', 'CSX', 'CVS', 'CVX', 'D', 'DAL', 'DAN',
            'DAR', 'DD', 'DE', 'DG', 'DGX', 'DHI', 'DIS', 'DLR', 'DLTR', 'DOV',
            'DOW', 'DPZ', 'DTE', 'DUK', 'DVA', 'DVN', 'DXCM', 'EA', 'EBAY', 'ECL',
            ...
            'JNJ', 'JPM', 'K', 'KDP', 'KEY', 'KEYS', 'KHC', 'KIM', 'KLAC', 'KMB',
            'KMI', 'KO', 'KR', 'KVUE', 'L', 'LDOS', 'LEN', 'LH', 'LHX', 'LIN',
            'LKQ', 'LLY', 'LMT', 'LOW', 'LRCX', 'LULU', 'LUV', 'LVS', 'LW', 'LYB',
            'LYV', 'MA', 'MAA', 'MAR', 'MAS', 'MCD', 'MCHP', 'MCK', 'MCO', 'MDLZ',
            'MDT', 'MET', 'META', 'MGM', 'MHK', 'MKC', 'MKTX', 'MLM', 'MMC', 'MNST',
            ...
            'PYPL', 'QCOM', 'QQQ', 'QRVO', 'RCL', 'REG', 'REGN', 'RF', 'RHI', 'RJF',
            'RL', 'RMD', 'ROK', 'ROL', 'ROP', 'ROST', 'RSG', 'RTX', 'RVTY', 'SBAC',
            'SBUX', 'SEDG', 'SEE', 'SHW', 'SJM', 'SLB', 'SNA', 'SNPS', 'SO', 'SPG',
            'SPGI', 'SRE', 'A', 'AAL', 'VIXM'
        ]
        
        # Convertir en format table_name
        table_names = [f"{s.lower()}_h4_data" for s in stocks]
        
        return sorted(stocks)
    except Exception as e:
        st.error(f"Erreur lors de la récupération des actions: {str(e)}")
        return []
