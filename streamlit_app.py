...
# Sélection du ticker à analyser
col_tech1, col_tech2 = st.columns([3, 9])
...
with col_tech2:
    ...
    hist_tech = get_historical_data(ticker_to_analyze, period_tech)
    
    if hist_tech is not None and len(hist_tech) > 0:
        ...
        fig_tech.add_trace(go.Scatter(
            ...
            name='EMA 12', line=dict(color='#00FFFF', width=1)
        ))
        fig_tech.add_trace(go.Scatter(
            x=df.index, y=df['EMA_26'],
            name='EMA 26', line=dict(color='#FF00FF', width=1)
        ))
        fig_tech.add_trace(go.Scatter(
            x=df.index, y=df['EMA_50'],
            name='EMA 50', line=dict(color='#FFA500', width=1.5)
        ))
        ...
        fig_tech.add_trace(go.Scatter(
            ...
            name='VWAP', line=dict(color='#FFFF00', width=1.5, dash='dot')
        ))
        
        fig_tech.update_layout(
            title=f"{ticker_to_analyze} - Technical Analysis - {period_tech.upper()}",
            paper_bgcolor='#000',
            plot_bgcolor='#111',
            font=dict(color='#FFAA00', size=10),
            xaxis=dict(gridcolor='#333', showgrid=True),
            yaxis=dict(gridcolor='#333', showgrid=True, title="Price"),
            hovermode='x unified',
            height=500,
        ...
        fig_macd.add_trace(go.Scatter(
            ...
            name='MACD', line=dict(color='#00FFFF', width=2)
        ))
        fig_macd.add_trace(go.Scatter(
            x=df.index, y=df['MACD_Signal'],
            name='Signal', line=dict(color='#FF00FF', width=2)
        ))
        fig_macd.add_trace(go.Bar(
            x=df.index, y=df['MACD_Hist'],
            name='Histogram',
            marker_color=['#00FF00' if x > 0 else '#FF0000' for x in df['MACD_Hist']]
        ))
        ...
        fig_rsi.add_trace(go.Scatter(
            x=df.index, y=df['RSI'],
            name='RSI', line=dict(color='#FFAA00', width=2),
            fill='tozeroy', fillcolor='rgba(255,170,0,0.2)'
        ))
        
        # Lignes de référence RSI
        fig_rsi.add_hline(y=70, line_dash="dash", line_color="#FF0000", opacity=0.5)
        fig_rsi.add_hline(y=30, line_dash="dash", line_color="#00FF00", opacity=0.5)
        ...
        fig_rsi.update_layout(
            ...
            font=dict(color='#FFAA00', size=10),
            xaxis=dict(gridcolor='#333', showgrid=True),
            yaxis=dict(gridcolor='#333', showgrid=True, range=[0, 100]),
            height=250
        )
        ...
        st.markdown("#### 📊 TECHNICAL INDICATORS - CURRENT VALUES")
        ...
        with tech_cols[2]:
            ...
            st.metric("BB Position", f"{bb_position:.1f}%", "Upper" if bb_position > 80 else "Lower" if bb_position < 20 else "Middle")
        
        with tech_cols[3]:
            st.metric("ATR (14)", f"{latest['ATR']:.2f}", "Volatility")
        
        with tech_cols[4]:
            vwap_signal = "🟢 ABOVE" if latest['Close'] > latest['VWAP'] else "🔴 BELOW"
            st.metric("Price vs VWAP", f"{((latest['Close']/latest['VWAP']-1)*100):+.2f}%", vwap_signal)
        
        with tech_cols[5]:
            ema_trend = "🟢 BULLISH" if latest['EMA_12'] > latest['EMA_26'] else "🔴 BEARISH"
            st.metric("EMA Trend", ema_trend.split()[1], ema_trend.split()[0])
        ...
        # Signal RSI
        if latest['RSI'] > 70:
            signals.append("🔴 **RSI Overbought** - Consider selling pressure")
        elif latest['RSI'] < 30:
            signals.append("🟢 **RSI Oversold** - Potential buying opportunity")
        
        # Signal MACD
        if latest['MACD'] > latest['MACD_Signal'] and previous['MACD'] <= previous['MACD_Signal']:
            signals.append("🟢 **MACD Bullish Crossover** - Potential uptrend starting")
        elif latest['MACD'] < latest['MACD_Signal'] and previous['MACD'] >= previous['MACD_Signal']:
            signals.append("🔴 **MACD Bearish Crossover** - Potential downtrend starting")
        
        # Signal EMA
        if latest['EMA_12'] > latest['EMA_26'] and latest['Close'] > latest['EMA_50']:
            signals.append("🟢 **Strong Uptrend** - Price above all EMAs")
        ...
        if latest['Close'] > latest['BB_Upper']:
            signals.append("⚠️ **Price above Upper Bollinger Band** - Possible reversion")
        elif latest['Close'] < latest['BB_Lower']:
            signals.append("⚠️ **Price below Lower Bollinger Band** - Possible bounce")
        ...
        if signals:
            ...
            st.info("📊 No strong signals detected - Market in consolidation")
        
        # ===== LÉGENDE DES INDICATEURS =====
        with st.expander("📖 INDICATORS LEGEND"):
            st.markdown("""
            **RSI (Relative Strength Index):**
            ...
            **Bollinger Bands:**
            ...
            - Bands width: Volatility measure
        ...
        st.error(f"❌ Impossible de charger les données pour {ticker_to_analyze}")
